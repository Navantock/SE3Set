...
class LightningMD17Dataset(LightningDataset):
    def __init__(self, 
                 root: str='dataset', 
                 label: str='aspirin',
                 ...
                 num_workers: int=0, 
                 **kwargs):
        self.dataset = Hypergraph_MD17(root, fragmentor_kwargs=fragmentor_kwargs, he_type=he_type, implicit_rc=implicit_rc, target=label, transform=transform, pre_transform=pre_transform, pre_filter=pre_filter)
        ...
        super(LightningMD17Dataset, self).__init__(train_dataset=trainset, 
                                                   val_dataset=valset, 
...
class Hypergraph_MD17(InMemoryDataset):
    ...
    def __init__(self, 
        ...
        assert target is not None, (
            ...
            f"'dataset_arg'. Available molecules are {', '.join(Hypergraph_MD17.available_molecules)} "
            "or 'all' to train on the combined dataset."
        )
        assert target in Hypergraph_MD17.available_molecules + ['all'], "Unknown data argument"

        if target == "all":
            target = ",".join(Hypergraph_MD17.available_molecules)
        ...
        super(Hypergraph_MD17, self).__init__(root, transform, pre_transform)
    ...
    def get(self, idx):
        ...
        return super(Hypergraph_MD17, self).get(idx - self.offsets[data_idx])

    @property
    def raw_file_names(self):
        return [Hypergraph_MD17.molecule_files[mol] for mol in self.molecules]
    ...
    def download(self):
        for file_name in self.raw_file_names:
            download_url(Hypergraph_MD17.raw_url + file_name, self.raw_dir)
