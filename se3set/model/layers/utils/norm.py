...
def get_norm_layer(norm_type):
    """Refer to https://github.com/atomicarchitects/equiformer/blob/master/nets/graph_attention_transformer.py#L39-L51
    """
    if norm_type == 'graph':
...
class EquivariantGraphNorm(nn.Module):
    ...
    Refer to https://github.com/atomicarchitects/equiformer/blob/master/nets/graph_norm.py#L8C1-L134C22
    ...
    def __init__(self, irreps, eps=1e-5, affine=True, reduce='mean', normalization='component'):
        super(EquivariantGraphNorm, self).__init__()
...
class EquivariantInstanceNorm(nn.Module):
    ...
    Note that the norm is invariant only for orthonormal representations.
    Irreducible representations `wigner_D` are orthonormal.
    Refer to https://github.com/atomicarchitects/equiformer/blob/master /nets/instance_norm.py#L8C1-L134C22
    ...
    """
    def __init__(self, irreps, eps=1e-5, affine=True, reduce='mean', normalization='component'):
        super(EquivariantInstanceNorm, self).__init__()
        self.irreps = Irreps(irreps)
        self.eps = eps
        self.affine = affine
...
class EquivariantLayerNormV2(nn.Module):
    """Refer to https://github.com/atomicarchitects/equiformer/blob/master/nets/layer_norm.py#L62C1-L152C22
    """
    def __init__(self, irreps, eps=1e-5, affine=True, normalization='component'):
        super(EquivariantLayerNormV2, self).__init__()
        self.irreps = Irreps(irreps)
        self.eps = eps
        self.affine = affine
...
class EquivariantLayerNormFast(nn.Module):
    """Refer to https://github.com/atomicarchitects/equiformer/blob/master/nets/fast_layer_norm.py#L9C1-L85C22
    """
    def __init__(self, irreps, eps=1e-5, affine=True, normalization='component'):
        super(EquivariantLayerNormFast, self).__init__()
        self.irreps = Irreps(irreps)
        self.eps = eps
        self.affine = affine
